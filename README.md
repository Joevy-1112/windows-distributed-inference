...
把一台 AMD 显卡的台式机和一台 NVIDIA 显卡的笔记本用千兆网线连起来，在 Windows 上跑通 Qwen2.5-14B 的分布式推理——不是双卡插在同一块主板上，是两台独立的 PC，走局域网 TCP 传张量。

2026 年 6 月 27 日，折腾了一整天。20K 上下文，纯独显，12-19 tok/s。

网上没见过同类记录——跨机异构 GPU 分布式推理，消费级硬件，Windows 环境。这篇文章记录整个过程。

---

## 硬件：两台 PC，一根网线

两台机器在同一个千兆局域网内，通过 TCP 传输张量——不是 PCIe，不是 NVLink，就是普通的家用以太网：
...
1. 两台独立 PC、一张千兆网线，异构 GPU 分布式推理在 Windows 上跑通了
...
6. 在 Linux 上同等硬件用 NCCL/Tensor Parallel 能跑得更好，但这不是 Linux 教程

台式机和笔记本、AMD 和 NVIDIA、Windows 和局域网——这套配置做到这个程度，应该是全网首例。
