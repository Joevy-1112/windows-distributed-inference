...
作为 EAB（电子辅助脑）项目的一部分，一加 7 Pro（SD855/8GB）成功运行 Qwen3.5-4B：

| 模式 | Prompt tok/s | Gen tok/s | 备注 |
|---|---|---|---|
| CPU (-ngl 0) | 12.3 | 5.0 | 纯 CPU 可用 |
| GPU (-ngl 99) | 11.9 | 5.0 | Adreno 640 无加速 |

Adreno 640 只支持 Vulkan 1.1，llama.cpp 需要 1.2，GPU 加速失败。纯 CPU 4B 12/5 tok/s 作为无头推理节点够用——消息推送 + 本地简单问答不依赖 GPU。
